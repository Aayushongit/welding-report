...
class SimulationConfig:
    ...
    # Heat source (Electron Beam Welding)
    Q_beam: float = 3000.0   # Beam power in W
    eta_ebw: float = 0.9     # Efficiency
    R_ebw: float = 0.001     # Beam radius in m
    H_ebw: float = 0.006     # Penetration depth in m
    v_weld: float = 0.01
    x_start: float = 0.02
    y_arc: float = 0.0
    ...
    # Welding process
    snapshot_time: float = 12.0  # Time in seconds to take a snapshot. -1 to disable.
...
def ebw_heat_source(X, Y, x_arc, y_arc, Q, eta, R, H):
    """
    Calculates the volumetric heat source for Electron Beam Welding (EBW).
    """
    r_sq = (X - x_arc)**2 + (Y - y_arc)**2
    Qvol = (Q * eta) / (np.pi * R**2 * H) * np.exp(-r_sq / R**2)
    return Qvol
...
def run_simulation(config: SimulationConfig):
    ...
    Q_beam, eta_ebw, R_ebw, H_ebw, v_weld, x_start, y_arc = config.Q_beam, config.eta_ebw, config.R_ebw, config.H_ebw, config.v_weld, config.x_start, config.y_arc
    ...
    print("Simulating Electron Beam Welding (EBW).")
    ...
    print(f"Power: {Q_beam * eta_ebw:.0f}W, Speed: {v_weld*1000:.1f}mm/s")
    ...
    frames = []
    video_filename = "ebw_simulation.mp4"
    ...
    for step in range(1, nt + 1):
        ...
        if x_arc <= Lx:
            Qvol = ebw_heat_source(X, Y, x_arc, y_arc, Q_beam, eta_ebw, R_ebw, H_ebw)
        else:
        ...
        if (step % max(1, nt // 100) == 0) or step == nt: # plot every few steps
            ...
            plt.clf()
            plt.title(f"EBW Temperature at t={t:.2f} s")
        ...
        if snapshot_time > 0 and t >= snapshot_time:
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Electron Beam Welding Simulation')
    parser.add_argument('--snapshot_time', type=float, default=12.0, help='Time in seconds to take a snapshot')
