...
def run_simulation(config: SimulationConfig):
    ...
    if snapshot_time > 0:
        ...
        t_end = Ly / v_weld + 10.0 # Adjusted for welding along y-axis
    nt = int(np.ceil(t_end / dt))

    print(f"Grid: {nx}x{ny}, Time steps: {nt}")
    print(f"Materials: {mat_1.name} | {mat_2.name}")
    print(f"Power: {Q_beam * eta_ebw:.0f}W, Speed: {v_weld*1000:.1f}mm/s (along y-axis)")
    ...
    for step in range(1, nt + 1):
        ...
        current_y_arc = config.y_arc + v_weld * t

        if current_y_arc <= Ly/2:
            Qvol = ebw_heat_source(X, Y, current_x_arc, current_y_arc, Q_beam, eta_ebw, R_ebw, H_ebw)
        else:
...
def main():
    config_file = "/media/dell/Hard Drive/Summer of code/MME/ERW-pipelines/welding-kar/ebw/config.json"
    ...
    config = SimulationConfig(
        ...
        eta_ebw=sim_params['eta_ebw'],
        R_ebw=sim_params['R_ebw'],
        H_ebw=sim_params['H_ebw'],
