...
class SimulationConfig:
    ...
    # Heat source (Laser Beam Welding)
    Q_beam: float  # Laser power in W
    eta_lbw: float # Absorption coefficient for surface
    R_lbw: float       # Radius of the beam on the surface in m
    H_lbw: float
    v_weld: float
    y_start: float
    x_arc: float
...
def laser_heat_flux(X, Y, x_arc, y_arc, Q, R_s, A_s):
    """
    Calculates the heat flux for Laser Beam Welding (LBW) using a Gaussian surface heat source model.
    """
    r_sq = (X - x_arc)**2 + (Y - y_arc)**2
    q = (2 * A_s * Q) / (np.pi * R_s**2) * np.exp(-2 * r_sq / R_s**2)
    return q
...
def run_simulation(config: SimulationConfig):
    ...
    Q_beam, eta_lbw, R_lbw, H_lbw, v_weld, y_start, x_arc = config.Q_beam, config.eta_lbw, config.R_lbw, config.H_lbw, config.v_weld, config.y_start, config.x_arc
    T0, h_conv, dt, theta, snapshot_time = config.T0, config.h_conv, config.dt, config.theta, config.snapshot_time
    ...
    midpoint = Lx / 2 # Changed to Lx/2 for welding along y-axis at x=0
    ...
    print("Simulating Laser Beam Welding (LBW).")

    t_end = (Ly - y_start) / v_weld + 10.0
    nt = int(np.ceil(t_end / dt))

    print(f"Grid: {nx}x{ny}, Time steps: {nt}")
    print(f"Materials: {mat_1.name} | {mat_2.name}")
    print(f"Power: {Q_beam:.0f}W, Speed: {v_weld*1000:.1f}mm/s")
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    video_filename = os.path.join(output_dir, "lbw_simulation.mp4")
    ...
    for step in range(1, nt + 1):
        ...
        if current_y_arc <= Ly/2:
            q_surf = laser_heat_flux(X, Y, x_arc, current_y_arc, Q_beam, R_lbw, eta_lbw)
        ...
        k = np.where(Y < midpoint, k_vec(T, mat_1.k, mat_1.T_crit, mat_1.T_melt), k_vec(T, mat_2.k, mat_2.T_crit, mat_2.T_melt))
        cp = np.where(Y < midpoint, cp_vec(T, mat_1.cp, mat_1.T_crit, mat_1.T_melt), cp_vec(T, mat_2.cp, mat_2.T_crit, mat_2.T_melt))
        rho = np.where(Y < midpoint, rho_vec(T, mat_1.rho, mat_1.T_crit, mat_1.T_melt), rho_vec(T, mat_2.rho, mat_2.T_crit, mat_2.T_melt))
        alpha = k / (rho * cp)
        ...
        if (step % max(1, nt // 100) == 0) or step == nt: # plot every few steps
            ...
            plt.clf()
            plt.title(f"LBW Temperature at t={t:.2f} s")
        ...
        if snapshot_time > 0 and t >= snapshot_time and not snapshot_taken:
            ...
            snapshot_taken = True
            print(f"Reached snapshot time at t={t:.2f}s. Stopping simulation.")
            break
...
def main():
    ...
    config = SimulationConfig(
        ...
        eta_lbw=sim_params['eta_lbw'],
        R_lbw=sim_params['R_lbw'],
        H_lbw=sim_params['H_lbw'],
        v_weld=sim_params['v_weld'],
        y_start=sim_params['y_start'],
        x_arc=sim_params['x_arc'],
        T0=sim_params['T0'],
