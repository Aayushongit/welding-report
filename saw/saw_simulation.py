...
class SimulationConfig:
    ...
    # Heat source (Submerged Arc Welding)
    Q_beam: float   # Heat input in W
    eta_saw: float
    R_saw: float     # Semi-axis of the ellipsoid in x
    H_saw: float
    v_weld: float
    ...
    # Welding process
    snapshot_time: float  # Time in seconds to take a snapshot. -1 to disable.
...
def saw_heat_flux(X, Y, x_arc, y_arc, Q, a, b):
    """
    Calculates the surface heat flux for Submerged Arc Welding (SAW).
    """
    xi = X - x_arc
    eta = Y - y_arc
    q_surf = (3 * Q) / (np.pi * a * b) * np.exp(-3 * xi**2 / a**2 - 3 * eta**2 / b**2)
    return q_surf
...
def save_plots(T, T_max, x, y, X, Y, T_crit, T_melt, T0, midpoint, time_history, T_history, monitor_pts, mat_1, mat_2, fusion_zone, HAZ_zone, dx, dy, nx, ny, snapshot_name=""):
    """Saves all the plots."""
    sns.set_theme(style="whitegrid") # Added seaborn theme
    output_dir = "output"
    ...
    plt.savefig(os.path.join(output_dir, f'enhanced_isotherms{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    fig2 = plt.figure(figsize=(20, 14))
    plt.clf()

    ax1_3d = fig2.add_subplot(221, projection='3d')
    ...
    fig2.colorbar(surf1, ax=ax1_3d, shrink=0.5, label='T (K)')
    ax1_3d.view_init(elev=25, azim=45)

    ax2_3d = fig2.add_subplot(222, projection='3d')
    ...
    fig2.colorbar(surf2, ax=ax2_3d, shrink=0.5, label='T (K)')
    ax2_3d.view_init(elev=30, azim=135)

    ax3_3d = fig2.add_subplot(223, projection='3d')
    ...
    ax4_3d = fig2.add_subplot(224, projection='3d')
    ...
    fig2.colorbar(surf4, ax=ax4_3d, shrink=0.5, label='|∇T| (K/m)')
    ax4_3d.view_init(elev=20, azim=60)

    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'enhanced_3d_views{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    fig3 = plt.figure(figsize=(18, 12))
    plt.clf()

    ax1_tc = fig3.add_subplot(221)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'enhanced_thermal_cycles{snapshot_name}.png'), dpi=300, bbox_inches='tight')
    print(f"Plots saved as enhanced_thermal_cycles{snapshot_name}.png")
    ...
    for temp in isotherm_levels_detailed:
        ...
        # Plot 1
    fig1 = plt.figure(figsize=(10, 8), num='1. Plot 1')
    plt.clf()
    ...
    # Plot 2
    fig2 = plt.figure(figsize=(10, 8), num='2. Plot 2')
    plt.clf()
    ...
    plt.savefig(os.path.join(output_dir, f'enhanced_isotherms{snapshot_name}.png'), dpi=300, bbox_inches='tight')
    # Plot 3
    fig3 = plt.figure(figsize=(10, 8), num='3. Plot 3')
    plt.clf()
    ...
    # Plot 4
    fig4 = plt.figure(figsize=(10, 8), num='4. Plot 4')
    plt.clf()
    ...
    # Plot 5
    fig5 = plt.figure(figsize=(10, 8), num='5. Plot 5')
    plt.clf()
    ...
    plt.savefig(os.path.join(output_dir, f'enhanced_3d_views{snapshot_name}.png'), dpi=300, bbox_inches='tight')
    # Plot 6
    fig6 = plt.figure(figsize=(10, 8), num='6. Plot 6')
    plt.clf()
    ...
    # Plot 7
    fig7 = plt.figure(figsize=(10, 8), num='7. Plot 7')
    plt.clf()
    ...
    # Plot 8
    fig8 = plt.figure(figsize=(10, 8), num='8. Plot 8')
    plt.clf()
    ...
    # Plot 9
    fig9 = plt.figure(figsize=(10, 8), num='9. Plot 9')
    plt.clf()
    ...
    plt.colorbar(im9, ax=ax9, label='Temperature (°C)')
                ...
                # Plot 10
    fig10 = plt.figure(figsize=(12, 10), num='10. 3D Plot 1')
    plt.clf()
    ...
    # Plot 11
    fig11 = plt.figure(figsize=(12, 10), num='11. 3D Plot 2')
    plt.clf()
    ...
    # Plot 12
    fig12 = plt.figure(figsize=(12, 10), num='12. 3D Plot 3')
    plt.clf()
    ...
    # Plot 13
    fig13 = plt.figure(figsize=(12, 10), num='13. 3D Plot 4')
    plt.clf()
    ...
    plt.savefig(os.path.join(output_dir, f'13_3d_plot{snapshot_name}.png'), dpi=300, bbox_inches='tight')
                # Plot 14
    fig14 = plt.figure(figsize=(10, 8), num='14. Thermal Cycle 1')
    plt.clf()
    ...
    # Plot 15
    fig15 = plt.figure(figsize=(10, 8), num='15. Thermal Cycle 2')
    plt.clf()
    ...
    # Plot 16
    fig16 = plt.figure(figsize=(10, 8), num='16. Thermal Cycle 3')
    plt.clf()
    ...
    # Plot 17
    fig17 = plt.figure(figsize=(10, 8), num='17. Thermal Cycle 4')
    plt.clf()
...
def run_simulation(config: SimulationConfig):
    ...
    Q_beam, eta_saw, R_saw, H_saw, v_weld, y_arc = config.Q_beam, config.eta_saw, config.R_saw, config.H_saw, config.v_weld, config.y_arc
    ...
    print("Simulating Submerged Arc Welding (SAW).")

    if snapshot_time > 0:
        t_end = snapshot_time
    else:
        t_end = Ly / v_weld + 10.0
    nt = int(np.ceil(t_end / dt))

    print(f"Grid: {nx}x{ny}, Time steps: {nt}")
    print(f"Materials: {mat_1.name} | {mat_2.name}")
    print(f"Power: {Q_beam:.0f}W, Speed: {v_weld*1000:.1f}mm/s (along y-axis)")
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    video_filename = os.path.join(output_dir, "saw_simulation.mp4")
    ...
    for step in range(1, nt + 1):
        ...
        current_y_arc = y_arc + v_weld * t

        if current_y_arc <= Ly/2: # Assuming welding from -Ly/2 to Ly/2
            q_surf = saw_heat_flux(X, Y, current_x_arc, current_y_arc, Q_beam, R_saw, H_saw)
        ...
        if (step % max(1, nt // 100) == 0) or step == nt: # plot every few steps
            ...
            plt.clf()
            plt.title(f"SAW Temperature at t={t:.2f} s")
...
def main():
    ...
    config = SimulationConfig(
        ...
        Q_beam=sim_params['Q_beam'],
        eta_saw=sim_params['eta_saw'],
        R_saw=sim_params['R_saw'],
        H_saw=sim_params['H_saw'],
        v_weld=sim_params['v_weld'],
        y_arc=sim_params['y_arc'],
        T0=sim_params['T0'],
