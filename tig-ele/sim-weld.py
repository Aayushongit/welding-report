...
class SimulationConfig:
    ...
    ff: float = 0.6
    fr: float = 1.4
    weld_direction: str = 'x' # 'x' for along x-axis, 'y' for along y-axis
    ...
    # Welding process
    weld_process: str = 'TIG'  # TIG or Electrode
    use_gas: bool = True
...
@njit
def goldak_heat_flux(X, Y, x_arc, y_arc, Q, a, b, cf, cr, ff, fr):
    """Optimized Goldak heat flux calculation using Numba."""
    ny, nx = X.shape
    q = np.zeros_like(X)

    a_sq = a * a
    b_sq = b * b
    coeff_f = (ff * Q) / (a * b * np.pi)
    coeff_r = (fr * Q) / (a * b * np.pi)

    for j in range(ny):
        for i in range(nx):
            xi = X[j, i] - x_arc
            eta = Y[j, i] - y_arc
            exp_arg = -xi * xi / a_sq - eta * eta / b_sq

            if xi >= 0:
                q[j, i] = coeff_f * np.exp(exp_arg)
            else:
                q[j, i] = coeff_r * np.exp(exp_arg)
...
@njit
def compute_material_properties_array(T_flat, X_flat, midpoint,
                                     mat_1_k, mat_1_cp, mat_1_rho, mat_1_T_crit, mat_1_T_melt,
                                     mat_2_k, mat_2_cp, mat_2_rho, mat_2_T_crit, mat_2_T_melt):
    """Compute material properties for all grid points efficiently."""
    n = len(T_flat)
    k_arr = np.empty(n)
    cp_arr = np.empty(n)
    rho_arr = np.empty(n)
    ...
    for i in range(n):
        if X_flat[i] < midpoint:
            k_arr[i] = get_k(T_flat[i], mat_1_k, mat_1_T_crit, mat_1_T_melt)
            cp_arr[i] = get_cp(T_flat[i], mat_1_cp, mat_1_T_crit, mat_1_T_melt)
            rho_arr[i] = get_rho(T_flat[i], mat_1_rho, mat_1_T_crit, mat_1_T_melt)
        else:
            k_arr[i] = get_k(T_flat[i], mat_2_k, mat_2_T_crit, mat_2_T_melt)
            cp_arr[i] = get_cp(T_flat[i], mat_2_cp, mat_2_T_crit, mat_2_T_melt)
            rho_arr[i] = get_rho(T_flat[i], mat_2_rho, mat_2_T_crit, mat_2_T_melt)

    return k_arr, cp_arr, rho_arr

def save_plots(T, T_max, x, y, X, Y, T_crit, T_melt, T0, midpoint, time_history, T_history, monitor_pts, mat_1, mat_2, fusion_zone, HAZ_zone, dx, dy, nx, ny, snapshot_name="", output_dir="output"):
    """Saves all the plots in separate windows."""
    ...
    fig1 = plt.figure(figsize=(10, 8), num='1. Detailed Temperature Isotherms')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'1_detailed_isotherms{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 2: Isotherm-Only View
    fig2 = plt.figure(figsize=(10, 8), num='2. Isotherm-Only View')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'2_isotherm_only{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 3: Color-Coded Isotherm Families
    fig3 = plt.figure(figsize=(10, 8), num='3. Color-Coded Isotherm Families')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'3_color_coded_isotherms{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 4: Temperature Gradient Magnitude
    fig4 = plt.figure(figsize=(10, 8), num='4. Temperature Gradient Magnitude')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'4_temperature_gradient{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 5: Fusion Zone & HAZ Regions
    fig5 = plt.figure(figsize=(10, 8), num='5. Fusion Zone & HAZ Regions')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'5_fusion_haz_zones{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 6: Centerline Temperature
    fig6 = plt.figure(figsize=(10, 8), num='6. Centerline Temperature')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'6_centerline_temperature{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 7: Transverse Temperature Profile
    fig7 = plt.figure(figsize=(10, 8), num='7. Transverse Temperature Profile')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'7_transverse_temperature{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 8: Weld Width Along Length
    fig8 = plt.figure(figsize=(10, 8), num='8. Weld Width Along Length')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'8_weld_width{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 9: Temperature in Celsius with Isotherms
    fig9 = plt.figure(figsize=(10, 8), num='9. Temperature (°C) with Isotherms')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'9_temperature_celsius{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 10: 3D Peak Temperature
    fig10 = plt.figure(figsize=(12, 10), num='10. 3D Peak Temperature')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'10_3d_peak_temperature{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 11: 3D with Isotherm Projections
    fig11 = plt.figure(figsize=(12, 10), num='11. 3D with Isotherm Projections')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'11_3d_isotherm_projections{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 12: 3D Zones Scatter
    fig12 = plt.figure(figsize=(12, 10), num='12. 3D Zones Scatter')
    plt.clf()
    ...
    ax3_3d.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'12_3d_zones_scatter{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 13: 3D Temperature Gradient
    fig13 = plt.figure(figsize=(12, 10), num='13. 3D Temperature Gradient')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'13_3d_temperature_gradient{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 14: Thermal Cycles
    fig14 = plt.figure(figsize=(10, 8), num='14. Thermal Cycles')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'14_thermal_cycles{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 15: Cooling Rates
    fig15 = plt.figure(figsize=(10, 8), num='15. Cooling Rates')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'15_cooling_rates{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 16: Final Temperature
    fig16 = plt.figure(figsize=(10, 8), num='16. Final Temperature')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'16_final_temperature{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    # Plot 17: Peak T vs Position
    fig17 = plt.figure(figsize=(10, 8), num='17. Peak T vs Position')
    plt.clf()
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, f'17_peak_temperature_vs_position{snapshot_name}.png'), dpi=300, bbox_inches='tight')

    print(f"All plots saved in {output_dir}/ directory")
    plt.show()

def run_simulation(config: SimulationConfig, output_dir="output"):
    ...
    V, I, eta, v_weld, x_start, y_arc_initial = config.V, config.I, config.eta, config.v_weld, config.x_start, config.y_arc
    a, b, cf, cr, ff, fr = config.a, config.b, config.cf, config.cr, config.ff, config.fr
    T0, h_conv, dt, theta = config.T0, config.h_conv, config.dt, config.theta
    weld_process, use_gas, snapshot_time, weld_direction = config.weld_process, config.use_gas, config.snapshot_time, config.weld_direction
    ...
    # Adjust parameters based on welding process
    if weld_process == 'TIG':
        print("Simulating TIG welding.")
        if use_gas:
            print("Using shielding gas.")
            eta = 0.75  # TIG is generally more efficient
        else:
            print("Not using shielding gas.")
            eta = 0.65
    elif weld_process == 'Electrode':
        print("Simulating Electrode welding.")
        eta = 0.85 # Electrode welding is generally less efficient
        if use_gas:
            print("Warning: Gas is not typically used with electrode welding.")
    ...
    if snapshot_time > 0:
        ...
        if weld_direction == 'x':
            t_end = (Lx - x_start) / v_weld + 10.0
        elif weld_direction == 'y':
            t_end = (Ly / 2 - (-Ly / 2)) / v_weld + 10.0 # Weld across the full y-domain
        else:
            raise ValueError("Invalid weld_direction. Must be 'x' or 'y'.")

    # If welding along y, x_start should be at the interface
    if weld_direction == 'y':
        x_start = Lx / 2
        y_arc_initial = -Ly / 2 # Start from the bottom of the y-domain
    elif weld_direction != 'x':
        raise ValueError("Invalid weld_direction. Must be 'x' or 'y'.")
    ...
    print(f"Power: {Q_total:.0f}W, Speed: {v_weld*1000:.1f}mm/s")
    ...
    T0_boundary = np.full(len(fixed_indices), T0)

    # Pre-compute constants
    dt_theta = theta * dt
    dt_one_minus_theta = (1 - theta) * dt
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    video_filename = os.path.join(output_dir, f"{weld_process.lower()}_simulation.mp4")
    fig_video = plt.figure(figsize=(8, 6))
    ...
    for step in range(1, nt + 1):
        ...
        if weld_direction == 'x':
            x_arc = x_start + v_weld * t
            y_arc = y_arc_initial
        elif weld_direction == 'y':
            x_arc = x_start # Fixed at the interface
            y_arc = y_arc_initial + v_weld * t
        else:
            raise ValueError("Invalid weld_direction. Must be 'x' or 'y'.")

        if (weld_direction == 'x' and x_arc <= Lx) or \
           (weld_direction == 'y' and y_arc <= Ly/2 and y_arc >= -Ly/2):
            q_surf = goldak_heat_flux(X, Y, x_arc, y_arc, Q_total, a, b, cf, cr, ff, fr)
            Qvol = q_surf / thickness
        else:
            Qvol = np.zeros_like(X)

        # Use optimized numba function for material properties
        Tvec[:] = T.ravel(order='C')
        k_arr, cp_arr, rho_arr = compute_material_properties_array(
            Tvec, X_flat, midpoint,
            mat_1.k, mat_1.cp, mat_1.rho, mat_1.T_crit, mat_1.T_melt,
            mat_2.k, mat_2.cp, mat_2.rho, mat_2.T_crit, mat_2.T_melt
        )
        alpha = k_arr / (rho_arr * cp_arr)

        # Build matrices more efficiently
        alpha_diag = sp.diags(alpha, format='csc')
        A_full = (I_mat - dt_theta * alpha_diag * L).tolil()
        B_full = (I_mat + dt_one_minus_theta * alpha_diag * L).tolil()
        ...
        # Compute RHS more efficiently
        RHS[:] = B.dot(Tvec)
        RHS += dt * (1.0 / (rho_arr * cp_arr)) * Qvec
        RHS[fixed_indices] = T0

        rhs_free = RHS[free_indices] - A_fF.dot(T0_boundary)
        ...
        if (step % max(1, nt // 100) == 0) or step == nt:  # Capture every few steps
            plt.figure(fig_video.number)
            plt.clf()
            plt.title(f"{weld_process} Temperature at t={t:.2f} s")
        ...
        if snapshot_time > 0 and t >= snapshot_time and not snapshot_taken:
            print(f"Reached snapshot time at t={t:.2f}s. Stopping simulation and saving results...")
            snapshot_taken = True
            break  # Exit the loop to save video and plots
    ...
    # Save final plots
    save_plots(T, T_max, x, y, X, Y, T_crit, T_melt, T0, midpoint, time_history, T_history, monitor_pts, mat_1, mat_2, fusion_zone, HAZ_zone, dx, dy, nx, ny, snapshot_name="_final", output_dir=output_dir)
...
def load_config_from_json(json_path):
    ...
    mat_1 = data['material_1']
    mat_2 = data['material_2']

    config = SimulationConfig(
        # Domain and mesh
        Lx=sim_params.get('Lx', 0.15),
        Ly=sim_params.get('Ly', 0.10),
        thickness=sim_params.get('thickness', 0.006),
        nx=sim_params.get('nx', 151),
        ny=sim_params.get('ny', 101),

        # Material 1
        mat_1_name=mat_1.get('name', 'Mild Steel'),
        mat_1_rho=mat_1.get('rho', 7850.0),
        mat_1_cp=mat_1.get('cp', 500.0),
        mat_1_k=mat_1.get('k', 45.0),
        mat_1_T_melt=mat_1.get('T_melt', 1811.0),
        mat_1_T_crit=mat_1.get('T_crit', 1273.0),

        # Material 2
        mat_2_name=mat_2.get('name', 'Stainless Steel 304'),
        mat_2_rho=mat_2.get('rho', 7900.0),
        mat_2_cp=mat_2.get('cp', 500.0),
        mat_2_k=mat_2.get('k', 16.3),
        mat_2_T_melt=mat_2.get('T_melt', 1723.0),
        mat_2_T_crit=mat_2.get('T_crit', 1273.0),

        # Heat source
        V=sim_params.get('V', 25.0),
        I=sim_params.get('I', 150.0),
        eta=sim_params.get('eta', 0.85),
        v_weld=sim_params.get('v_weld', 0.006),
        x_start=sim_params.get('x_start', 0.02),
        y_arc=sim_params.get('y_arc', 0.0),
        a=sim_params.get('a', 0.005),
        b=sim_params.get('b', 0.004),
        cf=sim_params.get('cf', 0.003),
        cr=sim_params.get('cr', 0.010),
        ff=sim_params.get('ff', 0.6),
        fr=sim_params.get('fr', 1.4),
        weld_direction=sim_params.get('weld_direction', 'y'),

        # Simulation parameters
        T0=sim_params.get('T0', 293.0),
        h_conv=sim_params.get('h_conv', 20.0),
        dt=sim_params.get('dt', 0.02),
        theta=sim_params.get('theta', 0.5),

        # Welding process
        weld_process=sim_params.get('weld_process', 'TIG'),
        use_gas=sim_params.get('use_gas', True),
        snapshot_time=sim_params.get('snapshot_time', -1.0)
    )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Welding Simulation')
    parser.add_argument('--config', type=str, default='config.json', help='Path to JSON configuration file')
    parser.add_argument('--weld_process', type=str, default=None, choices=['TIG', 'Electrode'], help='Welding process to simulate (overrides config)')
    parser.add_argument('--use_gas', action='store_true', default=None, help='Enable shielding gas (overrides config)')
    parser.add_argument('--no-gas', dest='use_gas', action='store_false', help='Disable shielding gas (overrides config)')
    parser.add_argument('--snapshot_time', type=float, default=None, help='Time in seconds to take a snapshot (overrides config)')
    parser.add_argument('--weld_direction', type=str, default=None, choices=['x', 'y'], help='Direction of welding (overrides config)')
    ...
    if args.weld_process is not None:
        config.weld_process = args.weld_process
    if args.use_gas is not None:
        config.use_gas = args.use_gas
    if args.snapshot_time is not None:
        config.snapshot_time = args.snapshot_time
    if args.weld_direction is not None:
        config.weld_direction = args.weld_direction

    run_simulation(config, output_dir=args.output_dir)
