...
def run_simulation(config: SimulationConfig):
    ...
    V, I, eta, v_weld, x_start, y_arc = config.V, config.I, config.eta, config.v_weld, config.x_start, config.y_arc
    ...
    weld_process, use_gas, snapshot_time = config.weld_process, config.use_gas, config.snapshot_time
    ...
    save_plots(T, T_max, x, y, X, Y, T_crit, T_melt, T0, midpoint, time_history, T_history, monitor_pts, mat_1, mat_2, fusion_zone, HAZ_zone, dx, dy, nx, ny)
    plt.show()

def main():
    """Main function to run the simulation."""
    parser = argparse.ArgumentParser(description='Welding Simulation')
    parser.add_argument('--weld_process', type=str, default='TIG', choices=['TIG', 'Electrode'], help='Welding process to simulate')
    parser.add_argument('--use_gas', action='store_true', help='Enable shielding gas')
    parser.add_argument('--no-gas', dest='use_gas', action='store_false', help='Disable shielding gas')
    parser.add_argument('--snapshot_time', type=float, default=-1.0, help='Time in seconds to take a snapshot')
    parser.set_defaults(use_gas=True)
